...
void geoParseObject(geointernalbuffertype_t *buf, geotype_t *geo){
  char tmp[11+1]; // toponimName has 11 characters
  int i = 0;
  if(buf->buffer[buf->index] == '\"'){
    ++buf->index;
    while(buf->buffer[buf->index] != '\"'){
      tmp[i] = buf->buffer[buf->index];
      ++i;
      ++buf->index;
    }
    tmp[i] = '\0';
    if(strncmp(tmp, "lng", sizeof("lng")-1) == 0){
      printf("%s %s\n", tmp, "lng");
    }
    else if(strncmp(tmp, "lat", sizeof("lat")-1) == 0){
      printf("%s %s\n", tmp, "lat");
    }
    else if(strncmp(tmp, "geonameId", sizeof("geonameId")-1) == 0){
      printf("%s %s\n", tmp, "geonameId");
    }
    else if(strncmp(tmp, "countrycode", sizeof("ountrycode")-1) == 0){
      printf("%s %s\n", tmp, "");
    }
    else if(strncmp(tmp, "name", sizeof("name")-1) == 0){
      printf("%s %s\n", tmp, "");
    }
    else if(strncmp(tmp, "fclName", sizeof("fclName")-1) == 0){
      printf("%s %s\n", tmp, "");
    }
    else if(strncmp(tmp, "toponymName", sizeof("toponymName")-1) == 0){
      printf("%s %s\n", tmp, "");
    }
    else if(strncmp(tmp, "fcodeName", sizeof("fcodeName")-1) == 0){
      printf("%s %s\n", tmp, "");
    }
    else if(strncmp(tmp, "wikipedia", sizeof("wikipedia")-1) == 0){
      printf("%s %s\n", tmp, "");
    }
    else if(strncmp(tmp, "fcl", sizeof("fcl")-1) == 0){
      printf("%s %s\n", tmp, "");
    }    
    else if(strncmp(tmp, "population", sizeof("population")-1) == 0){
      printf("%s %s\n", tmp, "");
    }
    else if(strncmp(tmp, "fcode", sizeof("fcode")-1) == 0){
      printf("%s %s\n", tmp, "");
    }    
  }
}
...
void geoParseInternalBuffer(geointernalbuffertype_t *buf, geotype_t *geo){
  if(strncmp(buf->buffer, "{\"geonames\":[{", 14) == 0){
